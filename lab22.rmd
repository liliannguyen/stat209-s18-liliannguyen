...
Use the `percentiles` function to detect the 3rd and 97th percentiles for
...
```{r}
pf$normal_speed <- fct_relevel(pf$speed, "normal")
```

```{r}
model <- lm_basic(perfectionism ~ 1 + normal_speed, data = pf)
reg_table(model, level = 0.95)
```
...
**Answer**: no - one doesn't include 0 
...
**Answer**: 97% was about an hour; 3% was about 6 minutes. Some may believe it is more serious than it actually is. Or maybe have to come back to it later, increasing the time. 
...
```{r}
model <- lm_basic(emotional_stability ~ 1 + sensitivity, data = pf)
reg_table(model, level = 0.95)
```
...
**Answer**: No. Because you would expect them to have a negative relationship but it has a postitive slope.  

Take the previous model and add the variables `speed`, `gender`, and
`country`, the latter lumped into 5 categories.

```{r}
model <- lm_basic(emotional_stability ~ 1 + sensitivity + speed + gender + fct_lump(country, n=5), data = pf)
reg_table(model, level = 0.95)
```
...
**Answer**: As the score for sensitivity increases, so does the score for emotional stability, as both slopes are positive. There appears to be no significant relationship between country and emotional stability. Nor does speed seem to be significant to emotional stability. 

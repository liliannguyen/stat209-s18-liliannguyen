...
Answer: There is a positive linear correlation between mass of the male bird and the mass of the egg.
...
```{r}
only_parrots <- filter(birds, type=="Parrot")
```
...
ggplot(birds, aes(male_mass, egg_mass)) +
  ...
  scale_x_log10() +
  scale_y_log10() +
  geom_point(color="red", data=only_parrots) +
  annotate("rect", xmin=32.7, xmax=184.7, ymin=2.75, ymax=12.50, fill="gray",
  alpha=0.15) +
  annotate("text", x = 108.7, y = 3, label = "Red Points are Parrots")
```
...
Answer: Yes, the line also displays a positive correlation, as I stated above. 
...
ggplot(birds, aes(male_mass, egg_mass)) +
  ...
  scale_y_log10() +
  geom_smooth(method = "lm", color = "black", se = FALSE, linetype = "dashed", size = 0.5) +
  annotate("text", x=218.7, y=95.00, label="Red-tailed tropicbird")
```
...
```{r}
big_eggs <- filter(birds, egg_mass>10)
```

```{r}
ggplot(big_eggs, aes(egg_mass, male_mass)) +
  geom_point() +
  scale_x_log10() +
  scale_y_log10() + 
  geom_point(aes(color=genus, size=clutch_size)) +
  geom_smooth(method = "lm", color = "red", se = FALSE, linetype = "dashed", size = 0.5) +
  xlab("Mass of Egg") +
  ylab("Mass of Male Bird") +
  ggtitle("Mass of Male Bird and Mass of Eggs, Greater than 10 g") 
```

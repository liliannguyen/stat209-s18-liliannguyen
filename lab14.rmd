...
```{r}
based_on_type <- group_summarize(birds, type)
```
...
```{r}
ggplot(based_on_type, aes(egg_mass_median, male_mass_median)) +
  geom_point() +
  scale_x_log10() +
  scale_y_log10()
```
...
```{r}
plus_five <- filter(based_on_type, n>=5)
```
...
ggplot(based_on_type, aes(egg_mass_median, male_mass_median)) +
  ...
  geom_point(aes(label=type), color="red", data=plus_five, size=2) +
  geom_text_repel(aes(label=type), color="red", data=plus_five, size=8) +
  scale_x_log10() +
  scale_y_log10() 
```
